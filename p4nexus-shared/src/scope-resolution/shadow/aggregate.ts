 ...
 * reads `.p4nexus/shadow-parity/latest.json` and renders.
 *
 * Related types — `ShadowAgreement`, `ShadowCallsite`, `ShadowDiff` — are
 * defined alongside `diffResolutions` in `./diff.ts` and re-exported
 * through the top-level `p4nexus-shared` barrel. Consumers import all
 * three from `p4nexus-shared`, not from this module.
 *
 ...
 *   `.p4nexus/shadow-parity/latest.json` and diffing snapshots is useful).
