...
- Renamed all internal paths from `kuzu` to `lbug` (storage: `.p4nexus/kuzu` → `.p4nexus/lbug`)
...
- **Optional skill generation** via `npx p4nexus analyze --skills` — generates AI agent skills from KuzuDB knowledge graph (#171) — @zander-raycraft
...
- Auto-reindex hook that runs `p4nexus analyze` after commits and merges, with automatic embeddings preservation (#205) — @L1nusB
...
- **Lazy CLI module loading**: All CLI subcommands now use `createLazyAction()` to defer heavy imports (tree-sitter, ONNX, KuzuDB) until invocation, significantly improving `p4nexus mcp` startup time (#207)
