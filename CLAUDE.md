...
- **Notes:** The P4Nexus CLI analyzer does not call an LLM.
...
Prefer **PreToolUse** hooks for hard gates (e.g. tests before `git_commit`). Adapt hook commands to `p4nexus/` npm scripts.
...
- **Call-resolution DAG:** See ARCHITECTURE.md § Call-Resolution DAG. Shared pipeline code in `p4nexus/src/core/ingestion/` must not name languages — use `LanguageProvider` hooks instead (see AGENTS.md).
- **P4Nexus:** `.claude/skills/p4nexus/`; MCP and indexed-repo rules live only in [AGENTS.md](AGENTS.md) (`p4nexus:start` … `p4nexus:end`). See **P4Nexus rules** below.
...
| 2026-03-24 | 1.2.0 | Removed duplicated p4nexus:start block and scope table; replaced with pointers to AGENTS.md. |
