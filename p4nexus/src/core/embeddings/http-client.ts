 ...
 * Returns null when P4NEXUS_EMBEDDING_URL + P4NEXUS_EMBEDDING_MODEL are unset.
 * Not cached — env vars are read fresh so late configuration takes effect.
 */
const readConfig = (): HttpConfig | null => {
  const baseUrl = process.env.P4NEXUS_EMBEDDING_URL;
  const model = process.env.P4NEXUS_EMBEDDING_MODEL;
  if (!baseUrl || !model) return null;

  const rawDims = process.env.P4NEXUS_EMBEDDING_DIMS;
  let dimensions: number | undefined;
  if (rawDims !== undefined) {
    const parsed = parseInt(rawDims, 10);
    if (Number.isNaN(parsed) || parsed <= 0) {
      throw new Error(`P4NEXUS_EMBEDDING_DIMS must be a positive integer, got "${rawDims}"`);
    }
    dimensions = parsed;
  }
  ...
  return {
    ...
    model,
    apiKey: process.env.P4NEXUS_EMBEDDING_API_KEY ?? 'unused',
    dimensions,
  };
};
    ...
    for (const item of items) {
      ...
      if (vec.length !== expected) {
        const hint = config.dimensions
          ? 'Update P4NEXUS_EMBEDDING_DIMS to match your model output.'
          : `Set P4NEXUS_EMBEDDING_DIMS=${vec.length} to match your model output.`;
        throw new Error(
...
export const httpEmbedQuery = async (text: string): Promise<number[]> => {
  ...
  if (embedding.length !== expected) {
    const hint = config.dimensions
      ? 'Update P4NEXUS_EMBEDDING_DIMS to match your model output.'
      : `Set P4NEXUS_EMBEDDING_DIMS=${embedding.length} to match your model output.`;
    throw new Error(
