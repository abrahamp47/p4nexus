 ...
 * Retries on lock errors (e.g., when `p4nexus analyze` is running).
 *
...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: p4nexus analyze`);
  }
  ...
  if (!shared) {
    ...
    // avoids lock conflicts when `p4nexus analyze` is writing.
