...
import { NODE_TABLES, REL_TABLE_NAME, REL_TYPES, EMBEDDING_TABLE_NAME } from 'p4nexus-shared';
...
export type { NodeTableName, RelType } from 'p4nexus-shared';
...
const _rawDims = parseInt(process.env.P4NEXUS_EMBEDDING_DIMS ?? '384', 10);
if (Number.isNaN(_rawDims) || _rawDims <= 0) {
  throw new Error(
    `P4NEXUS_EMBEDDING_DIMS must be a positive integer, got "${process.env.P4NEXUS_EMBEDDING_DIMS}"`,
  );
}
