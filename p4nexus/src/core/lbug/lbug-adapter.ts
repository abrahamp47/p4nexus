 ...
 * an exclusive lock on the LadybugDB file (e.g. `p4nexus analyze` or
 * `p4nexus serve` running at the same time).
 */
 ...
 * path should ignore this error — index creation is owned by `p4nexus
 ...
 * database is busy (e.g. `p4nexus analyze` holds the write lock).
 ...
 * the index is owned by `p4nexus analyze` (writable) and either already
