 ...
 * pinned explicitly by every caller, otherwise P4Nexus regresses:
 *
...
/**
 * Upper bound for any single P4Nexus LadybugDB file (graph index, group
 * bridge, install scratch, test fixture). 16 GiB is intentionally generous
 * for real-world code graphs (the P4Nexus self-index uses < 50 MiB) while
 * remaining far below any 64-bit OS mmap ceiling.
 *
 * Override with the `P4NEXUS_LBUG_MAX_DB_SIZE` environment variable when
...
export const LBUG_MAX_DB_SIZE: number = (() => {
  const raw = process.env.P4NEXUS_LBUG_MAX_DB_SIZE;
  if (raw) {
