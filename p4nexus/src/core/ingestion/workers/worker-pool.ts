...
export function resolveWorkerPoolOptions(
  ...
  return {
    ...
    subBatchMaxBytes:
      ...
      positiveInteger(process.env.P4NEXUS_WORKER_SUB_BATCH_MAX_BYTES) ??
      SUB_BATCH_MAX_BYTES,
    subBatchIdleTimeoutMs:
      positiveInteger(options.subBatchIdleTimeoutMs) ??
      positiveInteger(process.env.P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS) ??
        ...
        void fail(
          new Error(
            `Worker ${workerIndex} parse job idle timeout after ${job.timeoutMs / 1000}s ` +
              ...
              `--worker-timeout or P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS.`,
          ),
        );
        return false;
      };
