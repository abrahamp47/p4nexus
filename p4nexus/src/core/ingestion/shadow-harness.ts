 ...
 *     `.p4nexus/shadow-parity/` so the static dashboard at
 *     `p4nexus/shadow-parity-dashboard/` can render it offline.
 *
 * ## What it does NOT do
 *
 ...
 * ## Activation
 *
 * `P4NEXUS_SHADOW_MODE=1` (or `'true'`, `'yes'`, case-insensitive,
...
export interface ShadowHarness {
  /** `true` iff `P4NEXUS_SHADOW_MODE` is truthy. When `false`, `record()` is a no-op. */
...
/**
 * Construct a harness. Reads `P4NEXUS_SHADOW_MODE` at construction time
...
export function createShadowHarness(): ShadowHarness {
  const enabled = parseShadowModeEnv(process.env['P4NEXUS_SHADOW_MODE']);
...
/**
 * Env-var parser for `P4NEXUS_SHADOW_MODE`. Accepts the same truthy
