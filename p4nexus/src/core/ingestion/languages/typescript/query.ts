 ...
 *
 * Grammar version: tree-sitter-typescript pinned in p4nexus/package.json.
 *
...
;; + 57 useMemo) — all invisible to \`p4nexus_context\` /
;; \`p4nexus_impact\` for outgoing edges before this fix.
;;
 ...
 * (\`p4nexus_impact("Badge", direction: "upstream")\`) and call-graph
 * (\`p4nexus_context("Foo")\`) purposes JSX usage IS a call. Routing
