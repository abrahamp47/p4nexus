...
async function ensureBridgeReady(
  ...
  if (meta.version > 0 && meta.version !== BRIDGE_SCHEMA_VERSION) {
    return {
      error: `Bridge schema version mismatch (meta.json has ${meta.version}, expected ${BRIDGE_SCHEMA_VERSION}). Run p4nexus group sync for this group.`,
    };
  }
  ...
  try {
    ...
    return {
      error: `No bridge.lbug in this group directory. Run p4nexus group sync (schema ${BRIDGE_SCHEMA_VERSION}).`,
    };
  }
  const handle = await openBridgeDbReadOnly(groupDir);
  if (!handle) {
    return {
      error: `Could not open bridge.lbug read-only (schema ${BRIDGE_SCHEMA_VERSION}). Run p4nexus group sync.`,
    };
  }
  return { handle };
}
...
export async function runGroupImpact(
  ...
  const groupDir = getGroupDir(deps.p4nexusDir, name);
