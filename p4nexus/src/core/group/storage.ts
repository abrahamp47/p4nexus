...
export function getDefaultP4nexusDir(): string {
  return process.env.P4NEXUS_HOME || path.join(os.homedir(), '.p4nexus');
}

export function getGroupsBaseDir(p4nexusDir?: string): string {
  return path.join(p4nexusDir || getDefaultP4nexusDir(), 'groups');
}
...
export function getGroupDir(p4nexusDir: string, groupName: string): string {
  validateGroupName(groupName);
  return path.join(p4nexusDir, 'groups', groupName);
}
...
export async function listGroups(p4nexusDir?: string): Promise<string[]> {
  const groupsDir = getGroupsBaseDir(p4nexusDir);
  try {
...
export async function createGroupDir(
  ...
  const groupDir = getGroupDir(p4nexusDir, groupName);
