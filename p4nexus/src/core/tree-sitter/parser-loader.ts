...
const ISSUES_URL = 'https://github.com/abhigyanpatwari/P4Nexus/issues';
...
const SOURCES: Record<string, GrammarSource> = {
  ...
  [SupportedLanguages.Swift]: {
    ...
    unavailableNote:
      ...
      '`p4nexus/vendor/tree-sitter-swift`) failed to load. ' +
  ...
  [SupportedLanguages.Dart]: {
    ...
    unavailableNote:
      ...
      '`p4nexus/vendor/tree-sitter-dart`) failed to load. ' +
...
const logFailure = (key: string, result: LoadResult): void => {
  ...
  logged.add(key);
  const message = `[p4nexus] ${result.note} (${result.error.message})`;
