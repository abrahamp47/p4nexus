...
/**
 * Run the full P4Nexus analysis pipeline.
 *
...
export async function runFullAnalysis(
  ...
  if (existingMeta && !options.force && existingMeta.lastCommit === currentCommit) {
    ...
    if (currentCommit !== '') {
      ...
      return {
        ...
        // and `--skip-p4` paths unchanged (#1232/#1233 intent preserved).
        repoName:
  ...
  try {
    ...
    // Keep generated .p4nexus contents ignored without editing the user's root .gitignore.
    await ensureP4NexusIgnored(repoPath);
