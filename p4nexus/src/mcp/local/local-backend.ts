 ...
 *
 * Provides tool implementations using local .p4nexus/ indexes.
...
// AI context generation is CLI-only (p4nexus analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`P4Nexus [${context}]: ${msg}`);
}
 ...
 *
 * The `P4Nexus [query:timing] …` prefix keeps lines greppable; the
...
function logQueryTiming(query: string, phases: Record<string, number>): void {
  ...
  console.error(
    `P4Nexus [query:timing] query=${JSON.stringify(truncated)} totalMs=${totalMs} phases=${JSON.stringify(phases)}`,
  );
}
...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        console.error(
          `P4Nexus: "${entry.name}" has a stale KuzuDB index. Run: p4nexus analyze ${entry.path}`,
        );
      }
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: p4nexus analyze');
    }
   ...
   * launched from inside a sibling clone (typical for `npx p4nexus
  ...
  private async maybeWarnSiblingDrift(handle: RepoHandle): Promise<void> {
    ...
    this.warnedSiblingDrift.add(cacheKey);
    console.error(`P4Nexus: ${match.hint}`);
  }
  ...
  private async query(
    ...
    return {
      ...
      ...(!ftsUsed && {
        warning:
          'FTS extension unavailable - keyword search degraded. Run: p4nexus analyze --force to rebuild indexes.',
      }),
    };
  }
  ...
  private async bm25Search(
    ...
    try {
      ...
      console.error('P4Nexus: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
  ...
  private async semanticSearch(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    try {
      ...
      if (isVectorExtensionSupportedByPlatform()) {
        ...
        console.error(
          'P4Nexus [query:vector]: VECTOR extension not supported on this platform; using exact scan fallback',
        );
      }
  ...
  private async impact(
    ...
    try {
      ...
      return {
        ...
        impactedCount: 0,
        risk: 'UNKNOWN',
        suggestion: 'The graph query failed — try p4nexus context <symbol> as a fallback',
      };
    }
  }
  ...
  /**
   * MCP resource body for `p4nexus://group/{name}/contracts` (Issue #794).
   */
  ...
  /**
   * MCP resource body for `p4nexus://group/{name}/status` (Issue #794).
   */
