 ...
 *
 * Defines the tools that P4Nexus exposes to external AI agents.
...
export const P4NEXUS_TOOLS: ToolDefinition[] = [
  {
    name: 'list_repos',
    description: `List all indexed repositories available to P4Nexus.

Returns each repo's name, path, indexed date, last commit, and stats.

WHEN TO USE: First step when multiple repos are indexed, or to discover available repos.
AFTER THIS: READ p4nexus://repo/{name}/context for the repo you want to work with.
...
WHEN TO USE: Complex structural queries that search/explore can't answer. READ p4nexus://repo/{name}/schema first for the full schema.
...
AFTER THIS: Use impact() if planning changes, or READ p4nexus://repo/{name}/process/{processName} for full execution trace.
...
SERVICE: optional monorepo path prefix (case-sensitive path segments). When "repo" starts with "@", prefix-matches resolved symbol file paths; when a hit is outside the prefix, that member returns an empty payload for the symbol. Ignored for a normal indexed repo name.`,
  ...
  {
    name: 'detect_changes',
    description: `Analyze open/pending Perforce changes and find affected execution flows.
Maps p4 diff hunks to indexed symbols, then traces which processes are impacted.

WHEN TO USE: Before submitting — to understand what your changes affect. Pre-submit review, changelist preparation.
AFTER THIS: Review affected processes. Use context() on high-risk symbols. READ p4nexus://repo/{name}/process/{name} for full traces.
...
Returns: changed symbols, affected processes, and a risk summary.`,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        changelist: {
          type: 'string',
          description: 'Specific changelist number to analyze',
        },
        repo: {
          type: 'string',
          description: 'Repository name or path. Omit if only one repo is indexed.',
