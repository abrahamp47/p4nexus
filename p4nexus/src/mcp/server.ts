...
import { P4NEXUS_TOOLS } from './tools.js';
...
function getNextStepHint(toolName: string, args: Record<string, any> | undefined): string {
  ...
  switch (toolName) {
    case 'list_repos':
      return `\n\n---\n**Next:** READ p4nexus://repo/{name}/context for any repo above to get its overview and check staleness.`;
    ...
    case 'context':
      return `\n\n---\n**Next:** If planning changes, use impact({target: "${args?.name || '<name>'}", direction: "upstream"${repoParam}}) to check blast radius. To see execution flows, READ p4nexus://repo/${repoPath}/processes.`;

    case 'impact':
      return `\n\n---\n**Next:** Review d=1 items first (WILL BREAK). To check affected execution flows, READ p4nexus://repo/${repoPath}/processes.`;

    case 'detect_changes':
      return `\n\n---\n**Next:** Review affected processes. Use context() on high-risk changed symbols. READ p4nexus://repo/${repoPath}/process/{name} for full execution traces.`;

    case 'rename':
      return `\n\n---\n**Next:** Run detect_changes(${repoParam ? `{repo: "${repo}"}` : ''}) to verify no unexpected side effects from the rename.`;

    case 'cypher':
      return `\n\n---\n**Next:** To explore a result symbol, use context({name: "<name>"${repoParam}}). For schema reference, READ p4nexus://repo/${repoPath}/schema.`;
    ...
    case 'overview':
      return `\n\n---\n**Next:** To drill into an area, READ p4nexus://repo/${repoPath}/cluster/{name}. To see execution flows, READ p4nexus://repo/${repoPath}/processes.`;
...
Follow these steps:
1. READ \`p4nexus://repo/${repo || '{name}'}/context\` for codebase stats
2. READ \`p4nexus://repo/${repo || '{name}'}/clusters\` to see all functional areas
3. READ \`p4nexus://repo/${repo || '{name}'}/processes\` to see all execution flows
4. For the top 5 most important processes, READ \`p4nexus://repo/${repo || '{name}'}/process/{name}\` for step-by-step traces
...
export async function startMCPServer(backend: LocalBackend): Promise<void> {
  ...
  process.on('uncaughtException', (err) => {
    process.stderr.write(`P4Nexus MCP uncaughtException: ${err?.stack || err}\n`);
    shutdown(1);
  });
  process.on('unhandledRejection', (reason: any) => {
    process.stderr.write(`P4Nexus MCP unhandledRejection: ${reason?.stack || reason}\n`);
  });
