 ...
 * All resources use repo-scoped URIs: p4nexus://repo/{name}/context
 */
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'p4nexus://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'p4nexus://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'p4nexus://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'p4nexus://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'p4nexus://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'p4nexus://repo/{name}/process/{processName}',
      name: 'Process Trace',
      description: 'Step-by-step execution trace',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'p4nexus://group/{name}/contracts',
    ...
    {
      uriTemplate: 'p4nexus://group/{name}/status',
...
/** Query parameters for `p4nexus://group/{name}/contracts` */
...
/** Normalized parse result for P4Nexus MCP resource URIs */
export type ParsedP4nexusResource =
  | { kind: 'repos' }
  | { kind: 'setup' }
  | {
      kind: 'repo';
...
/**
 * Parse a P4Nexus resource URI (repos, setup, per-repo, or per-group templates).
 * Used by `readResource` and tests (round-trip / dispatch coverage).
 */
export function parseResourceUri(uri: string): ParsedP4nexusResource {
  if (uri === 'p4nexus://repos') return { kind: 'repos' };
  if (uri === 'p4nexus://setup') return { kind: 'setup' };
  ...
  if (u.hostname === 'group') {
    ...
    if (segments.length < 2) {
      throw new Error(
        `Invalid group resource URI (expected p4nexus://group/{name}/contracts or .../status): ${uri}`,
      );
    }
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: p4nexus analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# p4nexus_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: p4nexus analyze';
  }

  // Check staleness
  const repoPath = repo.repoPath;
  const lastCommit = repo.lastCommit || 'latest';
  ...
  lines.push('');
  lines.push('re_index: Run `npx p4nexus analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - p4nexus://repos: All indexed repositories');
  lines.push(`  - p4nexus://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - p4nexus://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - p4nexus://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - p4nexus://repo/${context.projectName}/process/{name}: Process trace`);
  lines.push(
    '  - p4nexus://group/{name}/contracts: Group contract registry (optional ?type=&repo=&unmatchedOnly=)',
  );
  lines.push('  - p4nexus://group/{name}/status: Group index / contract staleness');
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: p4nexus analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use p4nexus_query for deeper search.`,
      );
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: p4nexus analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use p4nexus_query for deeper search.`,
      );
    }
 ...
 * Useful for `p4nexus setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# P4Nexus\n\nNo repositories indexed. Run: `npx p4nexus analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# P4Nexus MCP — ${repo.name}`,
      '',
      `This project is indexed by P4Nexus as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`p4nexus://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`p4nexus://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`p4nexus://repo/${repo.name}/processes\` — All execution flows`,
      `- \`p4nexus://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
