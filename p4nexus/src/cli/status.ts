...
export const statusCommand = async () => {
  ...
  if (!isGitRepo(cwd)) {
    console.log('Not a Perforce workspace.');
    return;
  }
  ...
  if (!repo) {
    ...
    if (await hasKuzuIndex(storagePath)) {
      ...
      console.log('Run: p4nexus analyze   (rebuilds the index with LadybugDB)');
    } else {
      console.log('Repository not indexed.');
      console.log('Run: p4nexus analyze');
    }
    return;
  }
  ...
  console.log(`Status: ${isUpToDate ? '✅ up-to-date' : '⚠️ stale (re-run p4nexus analyze)'}`);
};
