 ...
 * Analyze Command
 *
 * Indexes a repository and stores the knowledge graph in .p4nexus/
 *
...
export interface AnalyzeOptions {
  ...
  skills?: boolean;
  verbose?: boolean;
  /** Skip AGENTS.md and CLAUDE.md p4nexus block updates. */
   ...
   * `P4NEXUS_MAX_FILE_SIZE` for the rest of the pipeline.
   */
  maxFileSize?: string;
...
export const analyzeCommand = async (inputPath?: string, options?: AnalyzeOptions) => {
  ...
  if (options?.verbose) {
    process.env.P4NEXUS_VERBOSE = '1';
  }

  if (options?.maxFileSize) {
    process.env.P4NEXUS_MAX_FILE_SIZE = options.maxFileSize;
  }
  ...
  if (options?.workerTimeout) {
    ...
    process.env.P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS = String(
  ...
  if (
    !setPositiveEnv(
      '--embedding-threads',
      'P4NEXUS_EMBEDDING_THREADS',
      options?.embeddingThreads,
    ) ||
    !setPositiveEnv(
      '--embedding-batch-size',
      'P4NEXUS_EMBEDDING_BATCH_SIZE',
      options?.embeddingBatchSize,
    ) ||
    !setPositiveEnv(
      '--embedding-sub-batch-size',
      'P4NEXUS_EMBEDDING_SUB_BATCH_SIZE',
  ...
  if (options?.embeddingDevice) {
    ...
    process.env.P4NEXUS_EMBEDDING_DEVICE = options.embeddingDevice;
  }
  ...
  } else if (options?.skipP4) {
    // --skip-p4: treat cwd as the index root, do not walk up to a parent workspace root.
  ...
  if (process.env.P4NEXUS_NO_GITIGNORE) {
    console.log(
      '  P4NEXUS_NO_GITIGNORE is set — skipping .gitignore (still reading .p4nexusignore)\n',
    );
  }
  ...
  try {
    ...
    try {
      ...
      console.log('\n  Tip: Run `p4nexus setup` to configure MCP for your editor.');
    }
    ...
    if (err instanceof RegistryNameCollisionError) {
      ...
      console.error(`    • Pick a different alias:  p4nexus analyze --name <alias>`);
      console.error(
        `    • Allow the duplicate:     p4nexus analyze --allow-duplicate-name  (leaves "-r ${err.registryName}" ambiguous)`,
      );
      console.error('');
      process.exitCode = 1;
      return;
    ...
    if (err instanceof AnalysisNotFinalizedError) {
      ...
      realStderrWrite(
        `\n  Diagnostic checklist:\n` +
          `    1. Re-run "p4nexus analyze" - transient native errors often clear on retry.\n` +
          ...
          `       and attach the trace to the P4Nexus issue tracker.\n\n`,
      );
      process.exitCode = 1;
      return;
    }
    ...
    if (
      ...
      console.error('    1. Add large vendored/generated directories to .p4nexusignore');
      ...
      // 'node.target'" crash happens inside npm *before* p4nexus code runs,
      ...
      console.error('    3. Reinstall p4nexus:     npm install -g p4nexus@latest');
      console.error('    4. Or try npx directly:    npx p4nexus@latest analyze');
      console.error('');
    } else if (
      ...
      console.error('  Suggestions:');
      console.error('    1. Reinstall:   npm install -g p4nexus@latest');
      console.error('    2. Clear cache: npm cache clean --force && npx p4nexus@latest analyze');
      console.error('');
    }
