...
export function registerGroupCommands(program: Command): void {
  ...
  group
    ...
    .action(async (name: string, opts: { force?: boolean }) => {
      const { createGroupDir, getDefaultP4nexusDir } = await import('../core/group/storage.js');
      const dir = await createGroupDir(getDefaultP4nexusDir(), name, opts.force);
      console.log(`Created group "${name}" at ${dir}`);
      console.log('Edit group.yaml to add repos, then run: p4nexus group sync ' + name);
    });
  ...
  group
    ...
    .action(async (groupName: string, groupPath: string, registryName: string) => {
      const { getGroupDir, getDefaultP4nexusDir } = await import('../core/group/storage.js');
      const { loadGroupConfig } = await import('../core/group/config-parser.js');
      const path = await import('node:path');
      const fs = await import('node:fs/promises');
      const groupDir = getGroupDir(getDefaultP4nexusDir(), groupName);
      ...
      console.log(`Run: p4nexus group sync ${groupName}`);
    });
  ...
  group
    ...
    .action(async (name?: string) => {
      ...
      if (!name) {
        ...
        if (groups.length === 0) {
          console.log('No groups configured. Create one with: p4nexus group create <name>');
          return;
        }
        console.log('Groups:');
      ...
      const { loadGroupConfig } = await import('../core/group/config-parser.js');
      const groupDir = getGroupDir(getDefaultP4nexusDir(), name);
      const config = await loadGroupConfig(groupDir);
  ...
  group
    ...
    .action(async (name: string) => {
      const { readContractRegistry, getGroupDir, getDefaultP4nexusDir } =
        await import('../core/group/storage.js');
