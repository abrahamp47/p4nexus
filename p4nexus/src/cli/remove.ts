 ...
 * Remove Command (#664)
 *
 * Delete the `.p4nexus/` index for a registered repo and unregister it
 * from the global registry (~/.p4nexus/registry.json). The target is
...
export const removeCommand = async (target: string, options?: { force?: boolean }) => {
  ...
  if (!options?.force) {
    console.log(`This will delete the P4Nexus index for: ${entry.name}`);
  ...
  // `<entry.path>/.p4nexus` subfolder. `~/.p4nexus/registry.json` is
