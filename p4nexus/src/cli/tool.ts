 ...
 *
 * Exposes P4Nexus tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely — invokes LocalBackend directly for minimal overhead.
 *
 * Usage:
 *   p4nexus query "authentication flow"
 *   p4nexus context --name "validateUser"
 *   p4nexus impact --target "AuthService" --direction upstream
 *   p4nexus cypher "MATCH (n:Function) RETURN n.name LIMIT 10"
 *
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('P4Nexus: No indexed repositories found. Run: p4nexus analyze');
    process.exit(1);
  }
  return _backend;
}
...
function output(data: any): void {
  ...
  try {
    ...
    if (err?.code === 'EPIPE') {
      // Consumer closed the pipe (e.g., `p4nexus cypher ... | head -1`)
...
export async function queryCommand(
  ...
  if (!queryText?.trim()) {
    console.error('Usage: p4nexus query <search_query>');
    process.exit(1);
  }
...
export async function contextCommand(
  ...
  if (!name?.trim() && !options?.uid) {
    console.error('Usage: p4nexus context <symbol_name> [--uid <uid>] [--file <path>]');
    process.exit(1);
  }
...
export async function impactCommand(
  ...
  if (!target?.trim()) {
    console.error('Usage: p4nexus impact <symbol_name> [--direction upstream|downstream]');
    process.exit(1);
  }
  ...
  try {
    ...
    output({
      ...
      target: { name: target },
      direction: options?.direction || 'upstream',
      suggestion: 'Try reducing --depth or using p4nexus context <symbol> as a fallback',
    });
    process.exit(1);
  }
}
...
export async function cypherCommand(
  ...
  if (!query?.trim()) {
    console.error('Usage: p4nexus cypher <cypher_query>');
    process.exit(1);
  }
