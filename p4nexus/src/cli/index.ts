...
program.name('p4nexus').description('P4Nexus local CLI and MCP server').version(pkg.version);
...
program
  ...
  .option('--skip-agents-md', 'Skip updating the p4nexus section in AGENTS.md and CLAUDE.md')
  .option('--no-stats', 'Omit volatile file/symbol counts from AGENTS.md and CLAUDE.md')
  .option(
    '--skip-p4',
    'Treat the provided path/cwd as the index root and skip Perforce workspace root discovery',
  )
  .option(
    '--name <alias>',
    'Register this repo under a custom name in ~/.p4nexus/registry.json ' +
  ...
  .addHelpText(
    ...
    '\nEnvironment variables:\n' +
      '  P4NEXUS_NO_GITIGNORE=1   Skip .gitignore parsing (still reads .p4nexusignore)\n' +
      '  P4NEXUS_MAX_FILE_SIZE=N  Override large-file skip threshold (KB). Default 512, max 32768.\n' +
      '  P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS=N  Worker idle timeout in milliseconds. Default 30000.\n' +
      '  P4NEXUS_WORKER_SUB_BATCH_MAX_BYTES=N  Worker job byte budget. Default 8388608.\n' +
      '  P4NEXUS_EMBEDDING_THREADS=N  Limit local ONNX CPU threads for --embeddings.\n' +
      '  P4NEXUS_SEMANTIC_EXACT_SCAN_LIMIT=N  Max embedding chunks for exact-scan fallback. Default 10000.\n' +
      '\nTip: `.p4nexusignore` supports `.gitignore`-style negation. Add e.g.\n' +
...
program
  .command('index [path...]')
  .description(
    'Register an existing .p4nexus/ folder into the global registry (no re-analysis needed)',
  )
  .option('-f, --force', 'Register even if meta.json is missing (stats will be empty)')
  .option('--allow-non-p4', 'Allow registering folders that are not Perforce workspaces')
...
program
  .command('clean')
  .description('Delete P4Nexus index for current repo')
...
program
  ...
  .description(
    'Delete the P4Nexus index for a registered repo (by alias, name, or absolute path). ' +
...
program
  ...
  .option('--api-key <key>', 'LLM API key or Azure api-key (saved to ~/.p4nexus/config.json)')
  .option(
    '--api-version <version>',
...
program
  ...
  .description('Map p4 diff hunks to indexed symbols and affected execution flows')
  .option('-s, --scope <scope>', 'What to analyze: default (default CL), all, or shelved', 'default')
  .option('-c, --changelist <cl>', 'Specific changelist number to analyze')
  .option('-r, --repo <name>', 'Target repository')
