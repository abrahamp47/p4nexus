 ...
 * Clean Command
 *
 * Removes the .p4nexus index from the current repository.
...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete P4Nexus indexes for ${entries.length} repo(s):`);
    ...
    for (const entry of entries) {
      ...
      // remove.ts. `~/.p4nexus/registry.json` is user-writable, so a
  ...
  if (!options?.force) {
    console.log(`This will delete the P4Nexus index for: ${repoName}`);
