...
export const mcpCommand = async () => {
  ...
  process.on('uncaughtException', (err) => {
    console.error(`P4Nexus MCP: uncaught exception — ${err.message}`);
    // Process is in an undefined state after uncaughtException — exit after flushing
    setTimeout(() => process.exit(1), 100);
  });
  process.on('unhandledRejection', (reason) => {
    const msg = reason instanceof Error ? reason.message : String(reason);
    console.error(`P4Nexus MCP: unhandled rejection — ${msg}`);
  });
  ...
  if (repos.length === 0) {
    console.error(
      'P4Nexus: No indexed repos yet. Run `p4nexus analyze` in a Perforce workspace — the server will pick it up automatically.',
    );
  } else {
    console.error(
      `P4Nexus: MCP server starting with ${repos.length} repo(s): ${repos.map((r) => r.name).join(', ')}`,
    );
  }
