 ...
 * Usage: p4nexus wiki [path] [options]
 */
...
export const wikiCommand = async (inputPath?: string, options?: WikiCommandOptions) => {
  ...
  if (inputPath) {
    ...
    if (!gitRoot) {
      console.log('  Error: Not inside a Perforce workspace\n');
      process.exitCode = 1;
      return;
    }
    repoPath = gitRoot;
  ...
  if (!isGitRepo(repoPath)) {
    console.log('  Error: Not a Perforce workspace\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (!meta) {
    console.log('  Error: No P4Nexus index found.');
    console.log('  Run `p4nexus analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    console.log('  Config saved to ~/.p4nexus/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      ...
      if (!llmConfig.apiKey && llmConfig.provider !== 'cursor') {
        ...
        console.log('  Set OPENAI_API_KEY or P4NEXUS_API_KEY environment variable,');
      ...
      if (choice === '5' && hasCursor) {
        ...
        const envKey = process.env.P4NEXUS_API_KEY || process.env.OPENAI_API_KEY || '';
        let azureKey: string;
        if (envKey) {
  ...
  try {
    ...
    if (options?.review && result.moduleTree) {
      ...
      if (choice === 'n' || choice === 'no') {
        console.log('\n  Generation cancelled. Run `p4nexus wiki` later to generate.\n');
        return;
      }
      ...
      if (choice === 'edit' || choice === 'e') {
        ...
        try {
          ...
          console.log('  Then run `p4nexus wiki` to continue.\n');
          return;
        }
      }
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `p4nexus wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
      if (process.env.P4NEXUS_VERBOSE) {
        console.error(err);
      }
    }
    process.exitCode = 1;
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync(
      'gh',
      ['gist', 'create', htmlPath, '--desc', 'Repository Wiki — generated by P4Nexus', '--public'],
