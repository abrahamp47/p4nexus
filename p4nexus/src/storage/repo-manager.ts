 ...
 * Repository Manager
 *
 * Manages P4Nexus index storage in .p4nexus/ at repo root.
 * Also maintains a global registry at ~/.p4nexus/registry.json
 ...
 * passed `p4nexus remove some-alias` and the alias misses every
...
/**
 * Shape of an entry in the global registry (~/.p4nexus/registry.json)
 */
...
const P4NEXUS_DIR = '.p4nexus';
const P4NEXUS_EXCLUDE_ENTRY = `${P4NEXUS_DIR}/`;

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .p4nexus storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), P4NEXUS_DIR);
};
 ...
 *
 * Returns:
 *   found        — true if .p4nexus/kuzu existed and was deleted
...
/**
 * Check if a path has a P4Nexus index
 */
...
/**
 * Find .p4nexus by walking up from a starting path
 */
 ...
 */
export const ensureP4NexusIgnored = async (repoPath: string): Promise<void> => {
...
const ensureGitInfoExclude = async (repoPath: string): Promise<void> => {
  ...
  if (excludes.includes(P4NEXUS_DIR) || excludes.includes(P4NEXUS_EXCLUDE_ENTRY)) return;

  const separator = content.length === 0 || content.endsWith('\n') ? '' : '\n';
  await fs.writeFile(excludePath, `${content}${separator}${P4NEXUS_EXCLUDE_ENTRY}\n`, 'utf-8');
};

// ─── Global Registry (~/.p4nexus/registry.json) ───────────────────────

/**
 * Get the path to the global P4Nexus directory
 */
export const getGlobalDir = (): string => {
  return process.env.P4NEXUS_HOME || path.join(os.homedir(), '.p4nexus');
};
 ...
 * Called after `p4nexus analyze` completes.
 *
...
export const registerRepo = async (
  ...
  if (opts?.name !== undefined) {
    ...
    if (existing && hasCustomAlias(existing, inferred)) {
      ...
      // so `--skip-p4` subdirs of unrelated parent workspaces keep using
 ...
 * in `~/.p4nexus/registry.json` (#1169).
 *
 * Why this exists: on Windows, `p4nexus analyze` has been observed to
 * exit cleanly (code 0) with `lbug.wal` written but no `meta.json`,
 * leaving the repo invisible to `p4nexus list`/`status` and downstream
  ...
  constructor(
    ...
    super(
      `Analysis did not finalize for ${repoPath}: ${detail}. ` +
        ...
        `Re-run "p4nexus analyze" — if the problem persists, inspect ` +
 ...
 *
 *   1. `meta.json` must exist at `<repoPath>/.p4nexus/meta.json`.
 ...
 * `storagePath` does NOT point at the expected `<entry.path>/.p4nexus`
 ...
 * cause is a corrupted or hand-edited `~/.p4nexus/registry.json`, and
  ...
  constructor(
    ...
    super(
      `Refusing to remove storage path for safety: expected ` +
        `"${expectedStoragePath}" under the repo's .p4nexus subfolder, ` +
        `but the registry entry has "${actualStoragePath}". ` +
        `This usually means the registry entry is corrupted or was ` +
        `hand-edited. Delete the entry manually from ~/.p4nexus/registry.json ` +
        `and re-run analyze.`,
    );
 ...
 * registry entry's `storagePath` is the canonical `<repo>/.p4nexus`
 ...
 *   - `~/.p4nexus/registry.json` is a plain-text user-writable file.
...
export const assertSafeStoragePath = (entry: RegistryEntry): void => {
  const expected = path.join(path.resolve(entry.path), '.p4nexus');
...
/**
 * Resolve a user-supplied target string (from `p4nexus remove <target>`
 ...
 * Optionally validates that each entry's .p4nexus/ still exists.
 */
export const listRegisteredRepos = async (opts?: {
  ...
  // Validate each entry still has a .p4nexus/ directory
...
// ─── Global CLI Config (~/.p4nexus/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.p4nexus/config.json
 */
...
/**
 * Save CLI config to ~/.p4nexus/config.json
 */
