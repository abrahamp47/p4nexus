 ...
 *
 * This module keeps the legacy export names (`isGitRepo`, `getGitRoot`, etc.)
 * so callers do not need to change signatures while the implementation is
 * Perforce-native.
 */
  ...
  try {
    ...
    for (const line of output.split('\n').filter((l) => l.trim())) {
      try {
        ...
        if (obj.change) return String(obj.change);
      } catch {
...
export const getRemoteUrl = (repoPath: string): string | undefined => {
  ...
  if (parts.length >= 2) {
    if (parts[0] === 'ssl' || parts[0] === 'tcp') parts[1] = parts[1].toLowerCase();
    else parts[0] = parts[0].toLowerCase();
    return parts.join(':');
  }
