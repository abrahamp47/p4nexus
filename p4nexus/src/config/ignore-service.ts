...
// hardcoded rule via a `!pattern` line in `.p4nexusignore` (#771) — same
...
/**
 * Load .gitignore and .p4nexusignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .p4nexusignore. Defaults to P4NEXUS_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
/**
 * Walk ancestor segments of `rel` and check whether `.p4nexusignore`
 ...
 * the user has an explicit `!__tests__/` line in `.p4nexusignore`.
 ...
 * - .gitignore / .p4nexusignore patterns (via `ignore` package)
 ...
 *
 * Precedence (#771): user's `.p4nexusignore` negation patterns take
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // User's .p4nexusignore negation takes precedence over hardcoded
      ...
      // Check .gitignore / .p4nexusignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      const rel = p.relative();
      // User's .p4nexusignore negation takes precedence (#771) — if the
      ...
      // Check against .gitignore / .p4nexusignore patterns.
