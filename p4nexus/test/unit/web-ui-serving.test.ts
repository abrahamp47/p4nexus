...
describe('landingPageHtml', () => {
  ...
  it('contains void background colour from p4nexus-web design tokens', () => {
    expect(html).toContain('#06060a');
  });

  it('contains surface card colour from p4nexus-web design tokens', () => {
    expect(html).toContain('#101018');
  });

  it('contains accent colour from p4nexus-web design tokens', () => {
  ...
  it('contains the build command in a terminal-style block', () => {
    expect(html).toContain('cd p4nexus-web');
    expect(html).toContain('npm run build');
  });

  it('contains the Vercel link with safe external attributes', () => {
    expect(html).toContain('https://p4nexus.vercel.app');
...
describe('resolveWebDistDir', () => {
  ...
  it('prefers P4NEXUS_WEB_DIST env var when set', async () => {
    const original = process.env.P4NEXUS_WEB_DIST;
    process.env.P4NEXUS_WEB_DIST = '/env/dist';
    try {
      accessMock.mockImplementation(async (p: string) => {
      ...
      if (original === undefined) {
        delete process.env.P4NEXUS_WEB_DIST;
      } else {
        process.env.P4NEXUS_WEB_DIST = original;
      }
    }
  });

  it('falls back to primary when P4NEXUS_WEB_DIST dir missing', async () => {
    const original = process.env.P4NEXUS_WEB_DIST;
    process.env.P4NEXUS_WEB_DIST = '/env/dist';
    try {
      accessMock.mockImplementation(async (p: string) => {
