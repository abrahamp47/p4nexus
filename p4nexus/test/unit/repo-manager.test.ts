...
describe('getStoragePath', () => {
  it('appends .p4nexus to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.p4nexus');
    expect(path.basename(result)).toBe('.p4nexus');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, lbugPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.p4nexus');
...
// ─── P4Nexus ignore rules (#1233) ─────────────────────────────────────

describe('ensureP4NexusIgnored (#1233)', () => {
  let tmpRepo: Awaited<ReturnType<typeof createTempDir>>;

  beforeEach(async () => {
    tmpRepo = await createTempDir('p4nexus-internal-gitignore-');
  });
  ...
  it('creates .p4nexus/.gitignore containing a catch-all ignore rule', async () => {
    await ensureP4NexusIgnored(tmpRepo.dbPath);

    await expect(
      fs.readFile(path.join(tmpRepo.dbPath, '.p4nexus', '.gitignore'), 'utf-8'),
    ).resolves.toBe('*\n');
  });
  ...
  it('adds .p4nexus/ to .git/info/exclude when the repo has a real .git directory', async () => {
    const excludePath = path.join(tmpRepo.dbPath, '.git', 'info', 'exclude');
    await fs.mkdir(path.dirname(excludePath), { recursive: true });

    await ensureP4NexusIgnored(tmpRepo.dbPath);

    await expect(fs.readFile(excludePath, 'utf-8')).resolves.toBe('.p4nexus/\n');
  });

  it('appends .p4nexus/ to .git/info/exclude once without disturbing existing rules', async () => {
    const excludePath = path.join(tmpRepo.dbPath, '.git', 'info', 'exclude');
    await fs.mkdir(path.dirname(excludePath), { recursive: true });
  ...
  it('keeps generated .p4nexus files out of git status', async () => {
    ...
    await ensureP4NexusIgnored(tmpRepo.dbPath);
    await fs.writeFile(path.join(tmpRepo.dbPath, '.p4nexus', 'meta.json'), '{}\n');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.p4nexus/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('p4nexus-config-test-');
...
//
// Each test isolates the global registry by pointing P4NEXUS_HOME at a
...
describe('registerRepo name override + collision guard (#829)', () => {
  ...
  let savedP4nexusHome: string | undefined;
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('p4nexus-registry-home-');
    tmpRepoA = await createTempDir('p4nexus-repo-a-');
    tmpRepoB = await createTempDir('p4nexus-repo-b-');
    savedP4nexusHome = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome.dbPath;
  });

  afterEach(async () => {
    if (savedP4nexusHome === undefined) delete process.env.P4NEXUS_HOME;
    else process.env.P4NEXUS_HOME = savedP4nexusHome;
  ...
  it('basename collisions without an explicit --name still register silently (backward-compat)', async () => {
    ...
    const parentA = await createTempDir('p4nexus-collide-parent-a-');
    const parentB = await createTempDir('p4nexus-collide-parent-b-');
...
describe('getInferredRepoName + registerRepo (#979 — git remote inference)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('p4nexus-registry-home-979-');
    savedP4nexusHome = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome.dbPath;
  });
  ...
  it('getInferredRepoName returns null when there is no .git directory', async () => {
    const tmp = await createTempDir('p4nexus-no-git-');
    try {
  ...
  it('getInferredRepoName returns null when origin is unset', async () => {
    const tmp = await createTempDir('p4nexus-no-origin-');
    try {
  ...
  it('getInferredRepoName returns the remote repo name when origin is set', async () => {
    const tmp = await createTempDir('p4nexus-with-origin-');
    try {
  ...
  it('registerRepo derives name from git remote when basename is generic (Gas-Town repro)', async () => {
    ...
    const root = await createTempDir('p4nexus-gastown-');
    try {
  ...
  it('two analyze calls of differently-remoted "rig" leaves no longer collide', async () => {
    ...
    const root = await createTempDir('p4nexus-gastown-2-');
    try {
  ...
  it('explicit --name still wins over remote inference', async () => {
    const tmp = await createTempDir('p4nexus-name-wins-');
    try {
  ...
  it('preserved alias still wins over remote inference on re-analyze', async () => {
    const tmp = await createTempDir('p4nexus-preserve-alias-');
    try {
      initGitRepo(tmp.dbPath, 'https://github.com/owner/from-remote.git');
  ...
  it('falls back to basename when no .git / no remote is available', async () => {
    const tmp = await createTempDir('p4nexus-fallback-basename-');
    try {
      const name = await registerRepo(tmp.dbPath, meta);
...
// ─── resolveRegistryEntry (#664 — p4nexus remove <target>) ──────────
//
...
// ~/.p4nexus. No P4NEXUS_HOME sandboxing needed. This also means the
  ...
  const entries: RegistryEntry[] = [
    {
      name: 'app',
      path: pathA,
      storagePath: `${pathA}${path.sep}.p4nexus`,
    ...
    },
    {
      name: 'app',
      path: pathB,
      storagePath: `${pathB}${path.sep}.p4nexus`,
      indexedAt: '2026-04-18T00:00:00.000Z',
      lastCommit: 'bbbbbbb',
    },
    {
      name: 'website',
      path: pathW,
      storagePath: `${pathW}${path.sep}.p4nexus`,
      indexedAt: '2026-04-18T00:00:00.000Z',
      lastCommit: 'ccccccc',
    },
  ];
  ...
  it('path match wins over name match (never ambiguous)', () => {
    ...
    const weird: RegistryEntry[] = [
      { ...entries[2] }, // 'website' at pathW
      {
        ...
        storagePath: `${prefix}elsewhere${path.sep}odd${path.sep}.p4nexus`,
        indexedAt: '2026-04-18T00:00:00.000Z',
        lastCommit: 'ddddddd',
      },
    ];
...
describe('resolveRegistryEntry backward-compat with non-canonical stored paths (#1003)', () => {
  it('matches a stored entry even when the target was passed in canonical form', async () => {
    ...
    const nonCanonical = realDir + path.sep + '.'; // e.g. /work/p4nexus/.
    ...
    const entries: RegistryEntry[] = [
      {
        ...
        path: nonCanonical,
        storagePath: path.join(nonCanonical, '.p4nexus'),
...
//
// Guard rail against destroying more than the `.p4nexus/` subfolder.
// `~/.p4nexus/registry.json` is user-writable plain text, so a
...
describe('assertSafeStoragePath (#1003)', () => {
  ...
  it('accepts the canonical <repo>/.p4nexus storage path', () => {
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(repoPath, '.p4nexus'),
    };
  ...
  it('rejects when storagePath is a sibling .p4nexus (right basename, wrong parent)', () => {
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(`${prefix}different${path.sep}repo`, '.p4nexus'),
    };
    expect(() => assertSafeStoragePath(entry)).toThrow(UnsafeStoragePathError);
  });
  ...
  it('UnsafeStoragePathError carries the original entry + expected + actual paths', () => {
    ...
    try {
      ...
      // Expected path is the canonical `<repo>/.p4nexus`.
      expect(err.expectedStoragePath).toBe(path.join(path.resolve(repoPath), '.p4nexus'));
  ...
  it('Windows: storagePath match is case-insensitive to match register/unregister semantics', () => {
    ...
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(repoPath.toUpperCase(), '.P4NEXUS'),
    };
...
describe('registerRepo worktree-aware basename fallback (#1259)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('p4nexus-registry-home-');
    tmpRepo = await createTempDir('p4nexus-canonical-repo-');
    savedP4nexusHome = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome.dbPath;
  });
