...
const ENV_KEYS = [
  'P4NEXUS_EMBEDDING_URL',
  'P4NEXUS_EMBEDDING_MODEL',
  'P4NEXUS_EMBEDDING_API_KEY',
  'P4NEXUS_EMBEDDING_DIMS',
] as const;
...
describe('HTTP embedding backend', () => {
  ...
  describe('MCP embedder', () => {
    ...
    it('returns true when HTTP environment variables are set', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://localhost:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
    ...
    it('reads custom dimensions from environment', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://localhost:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
      process.env.P4NEXUS_EMBEDDING_DIMS = '1024';
      const mod = await import('../../src/mcp/core/embedder.js');
      expect(mod.getEmbeddingDims()).toBe(1024);
    });

    it('retries query on transient server error', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://test:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
  ...
  describe('core embedder HTTP path', () => {
    it('sends correct request payload', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://test:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
      process.env.P4NEXUS_EMBEDDING_API_KEY = 'test-key';
    ...
    it('excludes API key from error messages', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://test:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
      process.env.P4NEXUS_EMBEDDING_API_KEY = 'secret-key-12345';
    ...
    it('throws on dimension mismatch when P4NEXUS_EMBEDDING_DIMS is set', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://test:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
      process.env.P4NEXUS_EMBEDDING_DIMS = '512';
  ...
  describe('dimension mismatch on query path', () => {
    ...
    it('throws with Set hint when P4NEXUS_EMBEDDING_DIMS is unset', async () => {
      process.env.P4NEXUS_EMBEDDING_URL = 'http://test:8080/v1';
      process.env.P4NEXUS_EMBEDDING_MODEL = 'test-model';
      ...
      const { embedText } = await import('../../src/core/embeddings/embedder.js');
      await expect(embedText('test')).rejects.toThrow('Set P4NEXUS_EMBEDDING_DIMS=768');
    });
  });
});
