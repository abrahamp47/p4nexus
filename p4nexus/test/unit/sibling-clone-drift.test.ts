...
describe('registry persists remoteUrl', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('p4nexus-sibling-home-');
    tmpRepo = await createTempDir('p4nexus-sibling-repo-');
    savedHome = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome.dbPath;
  });

  afterEach(async () => {
    if (savedHome === undefined) delete process.env.P4NEXUS_HOME;
    else process.env.P4NEXUS_HOME = savedHome;
...
describe('findSiblingClones', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('p4nexus-sibling-find-home-');
    savedHome = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome.dbPath;
  });
...
describe('checkCwdMatch', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('p4nexus-cwd-match-home-');
    savedHome = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome.dbPath;
  });
