...
// By default, execFileSync throws (simulating `which p4nexus` not found)
...
describe('setupClaudeCode', () => {
  ...
  it('writes win32 MCP entry with cmd wrapper', async () => {
    ...
    expect(config.mcpServers.p4nexus).toEqual({
      command: 'cmd',
  ...
  it('uses global binary path when p4nexus is on PATH', async () => {
    setPlatform('darwin');
    ...
    expect(config.mcpServers.p4nexus).toEqual({
      command: '/usr/local/bin/p4nexus',
      args: ['mcp'],
    });
  });

  it('falls back to npx when p4nexus is not on PATH', async () => {
    setPlatform('darwin');
  ...
  it('picks .cmd wrapper from Windows where output (multiple lines)', async () => {
    setPlatform('win32');
    // `where p4nexus` on Windows returns the POSIX script first, then .cmd
