...
describe('generateAIContextFiles', () => {
  ...
  beforeAll(async () => {
    ...
    storagePath = path.join(tmpDir, '.p4nexus');
  ...
  it('creates or updates CLAUDE.md with P4Nexus section', async () => {
    const stats = { nodes: 50, edges: 100, processes: 5 };
    await generateAIContextFiles(tmpDir, storagePath, 'TestProject', stats);

    const claudeMdPath = path.join(tmpDir, 'CLAUDE.md');
    const content = await fs.readFile(claudeMdPath, 'utf-8');
    expect(content).toContain('p4nexus:start');
    expect(content).toContain('p4nexus:end');
  ...
  it('keeps the load-bearing repo-specific sections in the CLAUDE.md block (#856)', async () => {
    ...
    expect(content).toContain('If any P4Nexus tool warns the index is stale');
    expect(content).toContain('## Always Do');
    expect(content).toContain('## Never Do');
    expect(content).toContain('## Resources');
    expect(content).toContain('p4nexus://repo/TestProject/context');
    expect(content).toContain('p4nexus-impact-analysis/SKILL.md');
    expect(content).toContain('p4nexus-refactoring/SKILL.md');
    expect(content).toContain('p4nexus-debugging/SKILL.md');
    expect(content).toContain('p4nexus-cli/SKILL.md');
  });
  ...
  it('keeps the CLAUDE.md P4Nexus block under the token-cost budget (#856)', async () => {
    ...
    const block = content.slice(
      content.indexOf('<!-- p4nexus:start -->'),
      content.indexOf('<!-- p4nexus:end -->'),
    );
  ...
  it('updates existing CLAUDE.md without duplicating', async () => {
    ...
    // Should only have one p4nexus section
    const starts = (content.match(/p4nexus:start/g) || []).length;
  ...
  it('installs skills files', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills', 'p4nexus');
    try {
  ...
  it('preserves inline marker references in prose and does not corrupt markdown (#1041)', async () => {
    ...
    const bugStorage = path.join(bugDir, '.p4nexus');
    await fs.mkdir(bugStorage, { recursive: true });

    const inlineProseLine =
      'See the `<!-- p4nexus:start --> … <!-- p4nexus:end -->` block in **[AGENTS.md](AGENTS.md)** for the canonical MCP tools, impact analysis rules, and index instructions.';
    ...
    try {
      ...
      expect((contentAfter.match(/<!-- p4nexus:start -->/g) || []).length).toBe(2);
      expect((contentAfter.match(/<!-- p4nexus:end -->/g) || []).length).toBe(2);
  ...
  it('matches section markers on files with CRLF line endings (#1041 cross-platform)', async () => {
    ...
    // `core.autocrlf`; when a section line ends `<!-- p4nexus:start
    ...
    const crlfStorage = path.join(crlfDir, '.p4nexus');
    ...
    const inlineProseLine =
      'See the `<!-- p4nexus:start --> … <!-- p4nexus:end -->` block in **[AGENTS.md](AGENTS.md)** for more.';
    const seeded = [
      '# Claude Code Rules',
      '',
      '## P4Nexus rules',
      ...
      '',
      '<!-- p4nexus:start -->',
      '# P4Nexus — Code Intelligence (stale stub)',
      '<!-- p4nexus:end -->',
      '',
    ].join('\r\n');
    ...
    try {
      ...
      expect((content.match(/<!-- p4nexus:start -->/g) || []).length).toBe(2);
      expect((content.match(/<!-- p4nexus:end -->/g) || []).length).toBe(2);
      ...
      expect(content).not.toContain('# P4Nexus — Code Intelligence (stale stub)');
    } finally {
