...
describe('resolveLLMConfig', () => {
  ...
  beforeEach(async () => {
    ...
    const configDir = path.join(tmpDir, '.p4nexus');
...
describe('--verbose flag', () => {
  const originalEnv = process.env.P4NEXUS_VERBOSE;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.P4NEXUS_VERBOSE;
    } else {
      process.env.P4NEXUS_VERBOSE = originalEnv;
    }
  });

  it('verboseLog writes to console when P4NEXUS_VERBOSE=1', async () => {
    ...
    expect(process.env.P4NEXUS_VERBOSE).toBe('1');

    consoleSpy.mockRestore();
  });

  it('verbose is off when P4NEXUS_VERBOSE is not set', () => {
    delete process.env.P4NEXUS_VERBOSE;
    expect(process.env.P4NEXUS_VERBOSE).toBeUndefined();
  });
});
