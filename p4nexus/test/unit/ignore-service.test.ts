...
// ─── .p4nexusignore negation can override hardcoded list (#771) ────
//
// Per @magyargergo's review: `.p4nexusignore` should honour
// `.gitignore`-style negation against the hardcoded DEFAULT_IGNORE_LIST.
// A `!__tests__/` line in `.p4nexusignore` must re-enable indexing of
...
//
// Locks in:
//   1. Default (no .p4nexusignore) — hardcoded list still blocks
...
describe('.p4nexusignore negation overrides hardcoded DEFAULT_IGNORE_LIST (#771)', () => {
  let tmpDir: string;
  ...
  it('default (no .p4nexusignore): __tests__ still blocked by hardcoded list', async () => {
  ...
  it('`!__tests__/` negation unlocks the directory and its descendants', async () => {
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), '!__tests__/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('`!__mocks__/` negation unlocks __mocks__ but NOT __tests__', async () => {
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), '!__mocks__/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('negation generalises — `!node_modules/` unlocks a different hardcoded entry', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), '!node_modules/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('standard `.gitignore` rules (no negation) still layer on top of hardcoded', async () => {
    // Pre-#771 behaviour: if .p4nexusignore says `my-dir/`, that dir
    // is ignored in addition to the hardcoded list. Non-negation
    // rules are unaffected by this PR.
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), 'my-dir/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('shouldIgnorePath (raw hardcoded check) is unchanged — wiki / external callers unaffected', async () => {
    ...
    // doesn't have access to per-repo `.p4nexusignore` config. Its
...
describe('loadIgnoreRules', () => {
  ...
  it('parses .p4nexusignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.p4nexusignore'));
  });
...
describe('createIgnoreFilter', () => {
  ...
  it('childrenIgnored respects negation patterns (exclude-all + whitelist)', async () => {
    // Reproduces https://github.com/abhigyanpatwari/P4Nexus/issues/596
    // Pattern: `*` (exclude all) + `!iOS/` + `!iOS/**` (whitelist iOS)
    await fs.writeFile(
      path.join(tmpDir, '.p4nexusignore'),
      '*\n!iOS/\n!iOS/**\n!backend/\n!backend/living_plan/\n!backend/living_plan/**\n',
    );
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('childrenIgnored respects negation patterns without trailing slash (!dir vs !dir/)', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), '*\n!iOS\n!iOS/**\n');
    const filter = await createIgnoreFilter(tmpDir);
...
describe('loadIgnoreRules — P4NEXUS_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when P4NEXUS_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');

    const original = process.env.P4NEXUS_NO_GITIGNORE;
    process.env.P4NEXUS_NO_GITIGNORE = '1';
    try {
      const ig = await loadIgnoreRules(tmpDir);
      ...
      if (original === undefined) {
        delete process.env.P4NEXUS_NO_GITIGNORE;
      } else {
        process.env.P4NEXUS_NO_GITIGNORE = original;
      }
      await fs.unlink(path.join(tmpDir, '.gitignore'));
    }
  });

  it('still reads .p4nexusignore when P4NEXUS_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.p4nexusignore'), 'vendor/\n');
