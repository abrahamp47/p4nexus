...
describe('git-clone', () => {
  describe('extractRepoName', () => {
    ...
    it('rejects URLs whose last segment is "..": prevents getCloneDir traversal escape', () => {
      ...
      // `getCloneDir('..')` = `~/.p4nexus/repos/..` = `~/.p4nexus/`, breaking
  ...
  describe('getCloneDir', () => {
    it('returns path under ~/.p4nexus/repos/', () => {
      const dir = getCloneDir('my-repo');
      expect(dir).toContain('.p4nexus');
    ...
    it('returned path is always a direct child of the clone root', () => {
      const cloneRoot = path.resolve(path.join(os.homedir(), '.p4nexus', 'repos'));
      const dir = getCloneDir('my-repo');
  ...
  describe('assertRemoteMatchesRequestedUrl', () => {
    ...
    beforeAll(async () => {
      fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), 'p4nexus-remote-match-'));
    ...
    it('rejects when the directory has no remote.origin', async () => {
      const noRemoteDir = await fs.mkdtemp(path.join(os.tmpdir(), 'p4nexus-no-remote-'));
      try {
        await new Promise<void>((resolve, reject) => {
  ...
  describe('getRemoteOriginUrl', () => {
    it('returns null for a directory that is not a git repository', async () => {
      const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'p4nexus-not-git-'));
      try {
