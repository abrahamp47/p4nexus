...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'p4nexus-hook.cjs');
const PLUGIN_HOOK = path.resolve(
  __dirname,
  '..',
  '..',
  '..',
  'p4nexus-claude-plugin',
  'hooks',
  'p4nexus-hook.js',
);

// ─── Test fixtures: temporary .p4nexus directory ───────────────────

let tmpDir: string;
let p4nexusDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-hook-test-'));
  p4nexusDir = path.join(tmpDir, '.p4nexus');
  fs.mkdirSync(p4nexusDir, { recursive: true });
...
function createGlobalRegistry(homeDir: string, marker: 'both' | 'registry' | 'repos' = 'both') {
  const registryDir = path.join(homeDir, '.p4nexus');
...
describe('Windows .cmd extension handling', () => {
  ...
  it('Plugin hook uses .cmd extension for Windows p4nexus binary', () => {
    const source = fs.readFileSync(PLUGIN_HOOK, 'utf-8');
    expect(source).toContain('p4nexus.cmd');
  });
});
...
describe('PostToolUse staleness detection (integration)', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: emits stale notification when HEAD differs from meta`, () => {
      ...
      fs.writeFileSync(
        path.join(p4nexusDir, 'meta.json'),
...
describe('Global registry lookup', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: PostToolUse stays silent for unindexed repo under global registry`, () => {
      const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-home-'));
    ...
    it(`${label}: PostToolUse emits stale for indexed repo under parent global registry`, () => {
      ...
      try {
        createGlobalRegistry(homeDir);
        fs.mkdirSync(path.join(repoDir, '.p4nexus'), { recursive: true });
        initGitRepo(repoDir);
        fs.writeFileSync(
          path.join(repoDir, '.p4nexus', 'meta.json'),
          JSON.stringify({ lastCommit: 'oldcommit', stats: {} }),
        );
...
describe('Linked git worktree resolution', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: PostToolUse emits stale from a linked worktree pointing at an indexed canonical repo`, () => {
      ...
      //   <root>/main-repo/.p4nexus/        (only here)
      //   <root>/main-repo-worktrees/feat/   (linked worktree, no .p4nexus)
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-worktree-'));
      ...
      try {
        ...
        fs.mkdirSync(path.join(mainRepo, '.p4nexus'), { recursive: true });
        fs.writeFileSync(
          path.join(mainRepo, '.p4nexus', 'meta.json'),
          JSON.stringify({ lastCommit: 'oldcommit', stats: {} }),
        );
        ...
        // Sanity: walking up from the worktree never reaches `.p4nexus`.
        expect(fs.existsSync(path.join(worktreePath, '.p4nexus'))).toBe(false);
        expect(fs.existsSync(path.join(path.dirname(worktreePath), '.p4nexus'))).toBe(false);
    ...
    it(`${label}: PostToolUse silent from a linked worktree when canonical repo has no .p4nexus`, () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-worktree-'));
      const mainRepo = path.join(root, 'main-repo');
      const worktreePath = path.join(root, 'main-repo-worktrees', 'feat');
      try {
        fs.mkdirSync(mainRepo, { recursive: true });
        initGitRepo(mainRepo);
        // Note: NO .p4nexus/ in the canonical repo.
...
describe('PostToolUse with missing/corrupt meta.json', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: emits stale when meta.json does not exist`, () => {
      const metaPath = path.join(p4nexusDir, 'meta.json');
