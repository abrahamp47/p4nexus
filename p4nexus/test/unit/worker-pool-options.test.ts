...
const ORIGINAL_TIMEOUT = process.env.P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS;
const ORIGINAL_MAX_BYTES = process.env.P4NEXUS_WORKER_SUB_BATCH_MAX_BYTES;

describe('resolveWorkerPoolOptions', () => {
  afterEach(() => {
    if (ORIGINAL_TIMEOUT === undefined) {
      delete process.env.P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS;
    } else {
      process.env.P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS = ORIGINAL_TIMEOUT;
    }

    if (ORIGINAL_MAX_BYTES === undefined) {
      delete process.env.P4NEXUS_WORKER_SUB_BATCH_MAX_BYTES;
    } else {
      process.env.P4NEXUS_WORKER_SUB_BATCH_MAX_BYTES = ORIGINAL_MAX_BYTES;
    }
  });

  it('reads worker timeout and byte budget from environment variables', () => {
    process.env.P4NEXUS_WORKER_SUB_BATCH_TIMEOUT_MS = '5000';
    process.env.P4NEXUS_WORKER_SUB_BATCH_MAX_BYTES = '4096';
