...
describe('hasGitDir', () => {
  ...
  it('returns true when .git directory exists', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-test-'));
    try {
  ...
  it('returns false for a non-existent path', async () => {
    const hasGitDir = await getHasGitDir();
    expect(hasGitDir('/tmp/__p4nexus_nonexistent_path__')).toBe(false);
  });
});
...
describe('isGitRepo', () => {
  ...
  it('returns false for a non-existent path', async () => {
    const { isGitRepo } = await import('../../src/storage/git.js');
    expect(isGitRepo('/tmp/__p4nexus_nonexistent__')).toBe(false);
  });
});
...
describe('getRemoteUrl', () => {
  const setupRepoWithRemote = (remoteUrl: string): string => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-remote-'));
...
//
// Critical for the worktree-naming bug: when `p4nexus analyze` runs from a
...
describe('getCanonicalRepoRoot', () => {
  it('returns null for a plain temp directory (not a git repo)', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-canonical-'));
    try {
  ...
  it('returns null for a non-existent path', async () => {
    const { getCanonicalRepoRoot } = await import('../../src/storage/git.js');
    expect(getCanonicalRepoRoot('/tmp/__p4nexus_canonical_nonexistent__')).toBeNull();
  });

  it('returns the repo root when called from a regular (non-worktree) checkout', async () => {
    const { getCanonicalRepoRoot } = await import('../../src/storage/git.js');
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-canonical-main-'));
    try {
      execSync('git init -q', { cwd: tmpDir });
  ...
  it('returns the CANONICAL repo root when called from inside a linked worktree (#1259)', async () => {
    ...
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-canonical-wt-'));
    try {
