 ...
 *
 * Tests: P4NEXUS_TOOLS from tools.ts
...
describe('P4NEXUS_TOOLS', () => {
  it('exports all tools (7 base + 3 route/tool/shape + 1 api_impact + 2 group)', () => {
    expect(P4NEXUS_TOOLS).toHaveLength(13);
  });

  it('contains all expected tool names', () => {
    const names = P4NEXUS_TOOLS.map((t) => t.name);
  ...
  it('each tool has name, description, and inputSchema', () => {
    for (const tool of P4NEXUS_TOOLS) {
  ...
  it('query is marked open-world because it may use external embeddings', () => {
    const queryTool = P4NEXUS_TOOLS.find((t) => t.name === 'query')!;
  ...
  it('rename tool requires new_name', () => {
    const renameTool = P4NEXUS_TOOLS.find((t) => t.name === 'rename')!;
  ...
  it('list_repos tool has no parameters', () => {
    const listTool = P4NEXUS_TOOLS.find((t) => t.name === 'list_repos')!;
  ...
  it('impact, query, and context expose optional service with minLength', () => {
    for (const n of ['impact', 'query', 'context'] as const) {
      const tool = P4NEXUS_TOOLS.find((t) => t.name === n)!;
  ...
  it('impact schema bounds match cross-impact validation ranges', () => {
    const impact = P4NEXUS_TOOLS.find((t) => t.name === 'impact')!;
  ...
  it('api_impact tool has no required parameters', () => {
    const apiImpactTool = P4NEXUS_TOOLS.find((t) => t.name === 'api_impact')!;
  ...
  it('route_map description defers to api_impact for pre-change analysis', () => {
    const routeMapTool = P4NEXUS_TOOLS.find((t) => t.name === 'route_map')!;
  ...
  it('shape_check description defers to api_impact for pre-change analysis', () => {
    const shapeCheckTool = P4NEXUS_TOOLS.find((t) => t.name === 'shape_check')!;
