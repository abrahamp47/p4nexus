...
describe('HttpRouteExtractor', () => {
  const tmpDir = path.join(os.tmpdir(), `p4nexus-http-extract-${Date.now()}`);
...
`,
  ...
  // ─── #1185: contract extractors must honour .p4nexusignore ─────────
  //
  // Pre-#1185 the source-scan path used a hardcoded
  // `[node_modules, .git, dist, build, vendor]` glob ignore array, so a
  // user's `.p4nexusignore` pattern (e.g. a Python venv `mentor_env/`,
  ...
  // `.p4nexusignore` (or `.gitignore`) prunes the glob.
  describe('respects .p4nexusignore (#1185)', () => {
    it('source-scan glob skips files matched by .p4nexusignore', async () => {
      const dir = path.join(tmpDir, 'p4nexusignore-honoured');
      fs.mkdirSync(path.join(dir, 'src/routes'), { recursive: true });
...
`,
      ...
      fs.writeFileSync(path.join(dir, '.p4nexusignore'), 'mentor_env/\n');
    ...
    // Pinned by the @claude review on PR #1247: above, only `.p4nexusignore`
    ...
    it('source-scan glob also skips files matched by `.gitignore` (no `.p4nexusignore`)', async () => {
...
`,
      ...
      // Note: NO .p4nexusignore — only `.gitignore`. This proves the
