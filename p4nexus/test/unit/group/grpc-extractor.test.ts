...
describe('GrpcExtractor', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'p4nexus-grpc-'));
...
export class AuthGateway {
  ...
  // ─── #1185: gRPC extractor must honour .p4nexusignore ──────────────
  //
  ...
  // `filesystem-walker.ts`) so any `.p4nexusignore` pattern is
  ...
  describe('respects .p4nexusignore (#1185)', () => {
    it('proto + source globs both skip files matched by .p4nexusignore', async () => {
...
service LeakedService {
  rpc Ping (PingRequest) returns (PingResponse);
      ...
      // `.p4nexusignore` filter this WOULD emit a `grpc::*/LeakedService`
      // consumer contract.
      writeFile(
...
stub = leaked_pb2_grpc.LeakedServiceStub(channel)`,
      ...
      writeFile('.p4nexusignore', 'mentor_env/\n');
...
describe('GrpcExtractor.extract ambiguous proto resolution', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'p4nexus-grpc-ambig-'));
    extractor = new GrpcExtractor();
  });
  afterEach(async () => {
    await fsp.rm(tmpDir, { recursive: true, force: true });
