...
describe('RustWorkspaceExtractor', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'p4nexus-rust-ws-'));
  });
  ...
  it('respects .p4nexusignore patterns', async () => {
    await writeFile(
      'lib/Cargo.toml',
      `[package]\nname = "mylib"\nversion = "0.1.0"\n\n[dependencies]\n`,
    );
    ...
    await writeFile('lib/.p4nexusignore', 'generated/\n');
    ...
    await writeFile('app/.p4nexusignore', 'generated/\n');
