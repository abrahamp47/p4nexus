...
describe('TopicExtractor', () => {
  ...
  beforeEach(() => {
    tmpDir = path.join(os.tmpdir(), `p4nexus-topic-${Date.now()}`);
...
await consumer.subscribe({ topic: 'order.placed' });`,
  ...
  // ─── #1185: topic extractor must honour .p4nexusignore ─────────────
  //
  ...
  // so any `.p4nexusignore` pattern excludes those files from contract
...
public void handleLeaked(ConsumerRecord<String, String> r) {}`,
    ...
    it('still prunes `*_test.go` even when .p4nexusignore is empty (wrapper preserves glob-level filter)', async () => {
