...
function tmpGroup(): { tmpDir: string; groupDir: string; cleanup: () => void } {
  const tmpDir = path.join(os.tmpdir(), `p4nexus-ci-${Date.now()}-${Math.random()}`);
  ...
  it('test_runGroupImpact_local_phase_thrown_exception_bubbles_as_top_level_error', async () => {
    ...
    try {
      const port: GroupToolPort = {
        ...
        impact: vi.fn(async () => {
          throw new Error('graph-load failure: .p4nexus missing');
        }),
        query: vi.fn(),
        impactByUid: vi.fn(),
        context: vi.fn(),
