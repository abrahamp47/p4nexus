...
describe('--skip-git CLI flag', () => {
  ...
  describe('--skip-git does not walk up to parent git repo (#1232)', () => {
    ...
    let p4nexusHome: string;

    function testEnv() {
      return {
        ...process.env,
        HOME: parentDir,
        P4NEXUS_HOME: p4nexusHome,
        P4NEXUS_LBUG_EXTENSION_INSTALL: 'never',
      };
    }

    function readRegistry(): Array<{ name: string; path: string }> {
      const registryPath = path.join(p4nexusHome, 'registry.json');
    ...
    function createTestStructure() {
      ...
      p4nexusHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gn-skip-git-home-'));
      initParentGitRepo();
    ...
    function cleanup() {
      ...
      if (p4nexusHome) {
        fs.rmSync(p4nexusHome, { recursive: true, force: true });
      }
    }
    ...
    it('keeps parent git status clean for --skip-git subdir analyze (#1233)', () => {
      ...
      try {
        ...
        expect(
          fs.readFileSync(path.join(parentDir, 'COOLIO', '.p4nexus', '.gitignore'), 'utf8'),
        ).toBe('*\n');
