...
describe('getMaxFileSizeBytes', () => {
  const ORIGINAL = process.env.P4NEXUS_MAX_FILE_SIZE;
  ...
  afterEach(() => {
    if (ORIGINAL === undefined) {
      ...
      process.env.P4NEXUS_MAX_FILE_SIZE = ORIGINAL;
    }
    warnSpy.mockRestore();
  });
  ...
  it('clamps values above the tree-sitter ceiling', () => {
    ...
    process.env.P4NEXUS_MAX_FILE_SIZE = String(aboveCeilingKb);
  ...
  it.each(['abc', '0', '-512', '1.5', 'NaN', ''])(
    ...
    (raw) => {
      if (raw === '') {
        ...
        process.env.P4NEXUS_MAX_FILE_SIZE = raw;
        expect(getMaxFileSizeBytes()).toBe(DEFAULT_MAX_FILE_SIZE_BYTES);
        expect(warnSpy).not.toHaveBeenCalled();
        return;
      }
  ...
  it('warns separately for distinct invalid values', () => {
    process.env.P4NEXUS_MAX_FILE_SIZE = 'abc';
    getMaxFileSizeBytes();
    process.env.P4NEXUS_MAX_FILE_SIZE = 'xyz';
    getMaxFileSizeBytes();
...
describe('getMaxFileSizeBannerMessage', () => {
  ...
  it('returns null when the env var equals the default (in KB)', () => {
    process.env.P4NEXUS_MAX_FILE_SIZE = String(DEFAULT_MAX_FILE_SIZE_BYTES / 1024);
    expect(getMaxFileSizeBannerMessage()).toBeNull();
  });
