...
const mockEnsureP4NexusIgnored = vi.fn();
...
vi.mock('../../src/storage/repo-manager.js', () => ({
  ...
  ensureP4NexusIgnored: mockEnsureP4NexusIgnored,
}));
...
vi.mock('../../src/storage/git.js', () => ({
  ...
  // older `.p4nexus/meta.json` files. The unit tests don't care
...
describe('indexCommand', () => {
  ...
  beforeEach(() => {
    ...
    mockGetStoragePaths.mockImplementation((repoPath: string) => ({
      storagePath: `${repoPath}/.p4nexus`,
      lbugPath: `${repoPath}/.p4nexus/lbug`,
      metaPath: `${repoPath}/.p4nexus/meta.json`,
    }));
    ...
    mockEnsureP4NexusIgnored.mockResolvedValue(undefined);
  ...
  it('fails when .p4nexus folder does not exist', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    mockAccess.mockRejectedValueOnce(new Error('missing .p4nexus'));
    ...
    expect(logSpy).toHaveBeenCalledWith(
      `  No .p4nexus/ folder found at: ${resolvedRepo}/.p4nexus`,
    );
  });
  ...
  it('fails when lbug database does not exist', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(logSpy).toHaveBeenCalledWith(
      '  .p4nexus/ folder exists but contains no LadybugDB index.',
    );
  });
  ...
  it('registers successfully with existing metadata', async () => {
    ...
    expect(mockEnsureP4NexusIgnored).toHaveBeenCalledTimes(1);
    expect(mockEnsureP4NexusIgnored).toHaveBeenCalledWith(resolvedRepo);
    expect(process.exitCode).toBeUndefined();
  });
  ...
  it('fails when multiple path parts do not resolve to a single existing path', async () => {
    ...
    expect(mockRegisterRepo).not.toHaveBeenCalled();
    expect(mockEnsureP4NexusIgnored).not.toHaveBeenCalled();
    expect(process.exitCode).toBe(1);
