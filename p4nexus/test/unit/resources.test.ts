...
describe('getResourceDefinitions', () => {
  ...
  it('includes repos resource', () => {
    const defs = getResourceDefinitions();
    const repos = defs.find((d) => d.uri === 'p4nexus://repos');
  ...
  it('includes setup resource', () => {
    const defs = getResourceDefinitions();
    const setup = defs.find((d) => d.uri === 'p4nexus://setup');
...
describe('getResourceTemplates', () => {
  ...
  it('includes context, clusters, processes, schema, cluster detail, process detail, group contracts/status', () => {
    ...
    expect(uris).toContain('p4nexus://repo/{name}/context');
    expect(uris).toContain('p4nexus://repo/{name}/clusters');
    expect(uris).toContain('p4nexus://repo/{name}/processes');
    expect(uris).toContain('p4nexus://repo/{name}/schema');
    expect(uris).toContain('p4nexus://repo/{name}/cluster/{clusterName}');
    expect(uris).toContain('p4nexus://repo/{name}/process/{processName}');
    expect(uris).toContain('p4nexus://group/{name}/contracts');
    expect(uris).toContain('p4nexus://group/{name}/status');
  });
...
describe('parseResourceUri', () => {
  it('parses group contracts without query', () => {
    const p = parseResourceUri('p4nexus://group/acme/contracts');
    expect(p).toEqual({
      kind: 'group',
      groupName: 'acme',
  ...
  it('parses nested group name and contracts query params', () => {
    const p = parseResourceUri(
      'p4nexus://group/acme/billing/contracts?type=http&repo=app%2Fapi&unmatchedOnly=true',
    );
  ...
  it('coerces unmatchedOnly false from string', () => {
    const p = parseResourceUri('p4nexus://group/g1/contracts?unmatchedOnly=false');
    expect(p.kind).toBe('group');
    if (p.kind !== 'group' || p.resourceType !== 'contracts') throw new Error('unexpected');
  ...
  it('parses group status', () => {
    const p = parseResourceUri('p4nexus://group/my/product/status');
    expect(p).toEqual({
      kind: 'group',
  ...
  it('round-trips repo URI like legacy regex', () => {
    const p = parseResourceUri('p4nexus://repo/my%20project/schema');
    expect(p).toEqual({
      kind: 'repo',
      repoName: 'my project',
      resourceType: 'schema',
  ...
  it('rejects unknown group resource tail', () => {
    expect(() => parseResourceUri('p4nexus://group/foo/bar')).toThrow('Unknown group resource');
  });
});
...
describe('readResource', () => {
  it('routes p4nexus://repos to listRepos', async () => {
    ...
    const result = await readResource('p4nexus://repos', backend);
  ...
  it('routes p4nexus://setup to setup resource', async () => {
    const backend = createMockBackend({
      repos: [
        {
          name: 'proj',
    ...
    const result = await readResource('p4nexus://setup', backend);
    expect(result).toContain('P4Nexus MCP');
  ...
  it('routes group contracts resource through backend', async () => {
    const backend = createMockBackend();
    const uri = 'p4nexus://group/g1/contracts?type=http&unmatchedOnly=true';
  ...
  it('routes group status resource through backend', async () => {
    const backend = createMockBackend();
    await readResource('p4nexus://group/acme/status', backend);
    expect(backend.readGroupStatusResource).toHaveBeenCalledWith('acme');
  });

  it('routes p4nexus://repo/{name}/context correctly', async () => {
    const backend = createMockBackend({
      context: {
    ...
    const result = await readResource('p4nexus://repo/test-project/context', backend);
  ...
  it('routes p4nexus://repo/{name}/schema to static schema', async () => {
    const backend = createMockBackend();
    const result = await readResource('p4nexus://repo/any/schema', backend);
    expect(result).toContain('P4Nexus Graph Schema');
    expect(result).toContain('CALLS');
    expect(result).toContain('IMPORTS');
  });

  it('routes p4nexus://repo/{name}/clusters correctly', async () => {
    const backend = createMockBackend({
      clusters: {
        clusters: [{ heuristicLabel: 'Auth', symbolCount: 10, cohesion: 0.9 }],
      },
    });
    const result = await readResource('p4nexus://repo/test/clusters', backend);
  ...
  it('routes p4nexus://repo/{name}/processes correctly', async () => {
    const backend = createMockBackend({
      processes: {
        processes: [{ heuristicLabel: 'LoginFlow', processType: 'intra_community', stepCount: 3 }],
      },
    });
    const result = await readResource('p4nexus://repo/test/processes', backend);
  ...
  it('routes p4nexus://repo/{name}/cluster/{clusterName} correctly', async () => {
    const backend = createMockBackend({
      clusterDetail: {
    ...
    const result = await readResource('p4nexus://repo/test/cluster/Auth', backend);
  ...
  it('handles cluster detail error', async () => {
    ...
    const result = await readResource('p4nexus://repo/test/cluster/Missing', backend);
    expect(result).toContain('Cluster not found');
  });

  it('routes p4nexus://repo/{name}/process/{processName} correctly', async () => {
    const backend = createMockBackend({
      processDetail: {
    ...
    const result = await readResource('p4nexus://repo/test/process/LoginFlow', backend);
  ...
  it('handles process detail error', async () => {
    ...
    const result = await readResource('p4nexus://repo/test/process/Missing', backend);
    expect(result).toContain('Process not found');
  });

  it('throws for unknown resource URI', async () => {
    const backend = createMockBackend();
    await expect(readResource('p4nexus://unknown', backend)).rejects.toThrow(
  ...
  it('throws for unknown repo-scoped resource type', async () => {
    const backend = createMockBackend();
    await expect(readResource('p4nexus://repo/test/nonexistent', backend)).rejects.toThrow(
      'Unknown resource',
    );
  });
  ...
  it('decodes URI-encoded repo names', async () => {
    const backend = createMockBackend();
    await readResource('p4nexus://repo/my%20project/schema', backend);
  ...
  it('decodes URI-encoded cluster names', async () => {
    ...
    await readResource('p4nexus://repo/test/cluster/Auth%20Module', backend);
