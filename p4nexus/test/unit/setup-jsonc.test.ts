...
describe('setupOpenCode — JSONC preservation', () => {
  ...
  "model": "test"
    ...
    expect(config.mcp.p4nexus).toBeDefined();
  ...
  it('updates existing p4nexus MCP entry without losing other keys', async () => {
    execFileSyncMock.mockReturnValueOnce('/usr/local/bin/p4nexus\n');
  ...
  "mcp": {
    "other": { "command": "keep" },
    "p4nexus": { "command": "old-p4nexus", "args": ["old"] }
  }
}`;
    await fs.writeFile(opencodeJsonPath(), jsonc, 'utf-8');
    ...
    expect(config.mcp.p4nexus).toEqual({
      type: 'local',
      command: ['/usr/local/bin/p4nexus', 'mcp'],
    });
  });
  ...
  it('does not wipe corrupt file content', async () => {
    ...
    expect(raw).not.toContain('p4nexus');
  });

  it('uses npx fallback format when p4nexus binary is not on PATH', async () => {
  ...
  "mcp": {}
    ...
    expect(config.mcp.p4nexus).toEqual({
      type: 'local',
      command: ['npx', '-y', 'p4nexus@latest', 'mcp'],
    });
  });
...
describe('setupCursor — JSONC preservation', () => {
  ...
  it('creates fresh mcp.json when missing', async () => {
    ...
    expect(config.mcpServers.p4nexus).toBeDefined();
  });
...
describe('installClaudeCodeHooks — JSONC preservation', () => {
  ...
  "hooks": {
    ...
    expect(config.hooks.PreToolUse[1].hooks[0].command).toContain('p4nexus-hook');
    expect(config.hooks.PostToolUse).toBeDefined();
  });

  it('does not add duplicate p4nexus-hook entries', async () => {
          ...
          PreToolUse: [
            {
              ...
              hooks: [{ type: 'command', command: 'node "p4nexus-hook.cjs"', timeout: 10 }],
            },
          ],
          PostToolUse: [
            {
