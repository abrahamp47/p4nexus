...
describe('filesystem-walker', () => {
  ...
  describe('.p4nexusignore support', () => {
    ...
    beforeAll(async () => {
      ...
      // Only .p4nexusignore, no .gitignore
      await fs.writeFile(path.join(nexusignoreDir, '.p4nexusignore'), 'local/\n');
    });
    ...
    it('excludes directories listed in .p4nexusignore', async () => {
  ...
  describe('combined .gitignore + .p4nexusignore', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(path.join(combinedDir, '.p4nexusignore'), 'local/\n');
    });
  ...
  describe('P4NEXUS_NO_GITIGNORE env var', () => {
    let envDir: string;
    ...
    it('includes gitignored directory when P4NEXUS_NO_GITIGNORE is set', async () => {
  ...
  describe('large file skip threshold (#991)', () => {
    ...
    const ORIGINAL_ENV = process.env.P4NEXUS_MAX_FILE_SIZE;
    ...
    beforeEach(() => {
      delete process.env.P4NEXUS_MAX_FILE_SIZE;
    ...
    afterEach(() => {
      if (ORIGINAL_ENV === undefined) {
        ...
        process.env.P4NEXUS_MAX_FILE_SIZE = ORIGINAL_ENV;
      }
      warnSpy.mockRestore();
    });
    ...
    it('includes the 600KB file when P4NEXUS_MAX_FILE_SIZE=1024', async () => {
      process.env.P4NEXUS_MAX_FILE_SIZE = '1024';
      const files = await walkRepositoryPaths(sizeDir);
      const paths = files.map((f) => f.path.replace(/\\/g, '/'));
      expect(paths).toContain(BIG_FILE);
    });

    it('falls back to default and warns once on invalid P4NEXUS_MAX_FILE_SIZE', async () => {
      process.env.P4NEXUS_MAX_FILE_SIZE = 'abc';
      const files = await walkRepositoryPaths(sizeDir);
      const paths = files.map((f) => f.path.replace(/\\/g, '/'));
      expect(paths).not.toContain(BIG_FILE);
    ...
    it('omits the "generated/vendored" suffix when threshold is overridden', async () => {
      process.env.P4NEXUS_MAX_FILE_SIZE = '1';
