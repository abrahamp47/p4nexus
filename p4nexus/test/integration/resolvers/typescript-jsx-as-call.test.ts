 ...
 * invokes the component at runtime. For `p4nexus_impact` and
 * `p4nexus_context` to give meaningful answers on `.tsx` codebases,
...
describe('TypeScript JSX-as-call CALLS edges', () => {
  ...
  it('combined HOF + JSX: const Wrapped = () => <Foo /> emits exactly one Wrapped → Foo', () => {
    ...
    // blast-radius counts in `p4nexus_impact`.
