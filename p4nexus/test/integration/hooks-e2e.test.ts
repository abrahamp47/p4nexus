 ...
 *
 * Tests the hook scripts with real git repos and .p4nexus directories.
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  describe('PostToolUse staleness detection', () => {
    it('detects stale index when meta.json lastCommit differs from HEAD', () => {
      ...
      expect(output!.additionalContext).toContain('npx p4nexus analyze');
    });
  ...
  describe('PreToolUse — silent without p4nexus CLI', () => {
    // PreToolUse tries to spawn `p4nexus augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `p4nexus augment` hangs and the 10s timeout kills the process.
  ...
  describe('unhappy paths', () => {
    it('handles corrupted meta.json (invalid JSON) without crashing', () => {
      fs.writeFileSync(path.join(p4nexusDir, 'meta.json'), 'THIS IS NOT JSON {{{');
    ...
    it('handles meta.json with missing lastCommit field', () => {
      fs.writeFileSync(path.join(p4nexusDir, 'meta.json'), JSON.stringify({ stats: {} }));
  ...
  describe('directory without .p4nexus', () => {
    // The hook walks up 5 parent directories looking for .p4nexus.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .p4nexus could exist in any ancestor.
    let noP4NexusDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .p4nexus
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-p4nexus-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      noP4NexusDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(noP4NexusDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: noP4NexusDir, stdio: 'pipe' });
    });
    ...
    afterAll(() => {
      ...
      const base = path.join(
        root,
        path.basename(path.resolve(noP4NexusDir, '..', '..', '..', '..', '..', '..')),
      );
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .p4nexus directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .p4nexus directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
