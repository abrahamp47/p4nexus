...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'p4nexus-grp-impact-int-'));
...
describe('group impact integration', () => {
  ...
  it('runs happy-path stub against fixture group (stops before bridge when no symbol UIDs)', async () => {
    const prev = process.env.P4NEXUS_HOME;
    process.env.P4NEXUS_HOME = tmpHome;
    try {
      const svc = new GroupService(stubPort());
      ...
      if (prev === undefined) delete process.env.P4NEXUS_HOME;
      else process.env.P4NEXUS_HOME = prev;
    }
  });
});
