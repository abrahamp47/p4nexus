 ...
 * - analyzeCommand: verify pipeline runs and creates .p4nexus/ output
 *
 ...
 * isolate the global registry via P4NEXUS_HOME so they don't touch the
 * developer / CI agent's real ~/.p4nexus/registry.json (#829).
 */
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    ...
    // Successful analyze should create .p4nexus/ output directory
    const p4nexusDir = path.join(MINI_REPO, '.p4nexus');
    expect(fs.existsSync(p4nexusDir)).toBe(true);
    expect(fs.statSync(p4nexusDir).isDirectory()).toBe(true);
    expect(fs.existsSync(path.join(MINI_REPO, '.gitignore'))).toBe(false);
    expect(fs.readFileSync(path.join(p4nexusDir, '.gitignore'), 'utf-8')).toBe('*\n');
  }, 60_000);
  ...
  it('analyze persists meta.json AND a matching registry entry (#1169)', () => {
    ...
    try {
      const result = runCliWithEnv(['analyze'], repo, { P4NEXUS_HOME: gnHome }, 60000);
      ...
      const metaPath = path.join(repo, '.p4nexus', 'meta.json');
      expect(
  ...
  it('already-up-to-date analyze fails when registry entry is missing (#1169)', () => {
    ...
    try {
      const first = runCliWithEnv(['analyze'], repo, { P4NEXUS_HOME: gnHome }, 60000);
      expect(
        first.status,
        [
      ...
      const second = runCliWithEnv(['analyze'], repo, { P4NEXUS_HOME: gnHome }, 60000);
      expect(
        second.status,
        [
  ...
  //   1. `analyze --name X` persists the alias to ~/.p4nexus/registry.json
  ...
  describe('remove <target> (#664)', () => {
    it('dry-run lists, --force deletes, missing target is a no-op warning', () => {
      ...
      try {
        ...
        const r2 = runCliWithEnv(['remove', 'alias-a'], parentA, { P4NEXUS_HOME: gnHome }, 15000);
        if (r2.status === null) return;
        ...
        const r4 = runCliWithEnv(['remove', 'alias-a'], parentA, { P4NEXUS_HOME: gnHome }, 15000);
        if (r4.status === null) return;
    ...
    it('refuses to proceed when a registry entry points storagePath outside <repo>/.p4nexus (#1003)', () => {
      // Regression guard for the safety gap flagged by @magyargergo on
      // PR #1003: `~/.p4nexus/registry.json` is a user-writable JSON
      ...
      // `<entry.path>/.p4nexus`. We verify:
      //   1. Exit code 1 with the actionable "registry entry corrupted"
      //      hint.
      //   2. The .p4nexus/ storage dir is UNTOUCHED.
      ...
      try {
        ...
        expect(fs.existsSync(path.join(repo, '.p4nexus'))).toBe(true);
        ...
        // Repo + .p4nexus dir + .git dir must all still exist — the
        ...
        expect(fs.existsSync(path.join(repo, '.p4nexus')), 'storage dir must survive').toBe(true);
  ...
  // the rest. We verify:
  //   1. The poisoned entry is NOT deleted (working tree + .p4nexus
  ...
  //      .p4nexus dir AND its registry entry are gone).
  describe('clean --all with a poisoned registry entry (#1003)', () => {
    it('skips poisoned entries, cleans valid ones, never deletes the working tree', () => {
      ...
      try {
        ...
        // Sanity: both working trees and .p4nexus dirs still exist.
        expect(fs.existsSync(repoBad)).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.p4nexus'))).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.git'))).toBe(true);
        expect(fs.existsSync(path.join(repoGood, '.p4nexus'))).toBe(true);
        ...
        // Poisoned repo: working tree + .p4nexus + .git all SURVIVE.
        expect(fs.existsSync(repoBad), 'poisoned repo working tree must survive').toBe(true);
        expect(
          fs.existsSync(path.join(repoBad, '.p4nexus')),
          'poisoned repo .p4nexus must survive (guard refused to rm repo root)',
        ).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.git')), '.git must survive').toBe(true);

        // Good repo: its .p4nexus IS gone (cleanup succeeded despite
        // the poisoned sibling entry — per-entry error tolerance is
        // preserved).
        expect(
          fs.existsSync(path.join(repoGood, '.p4nexus')),
          'good repo .p4nexus should be cleaned',
        ).toBe(false);
        // But the good repo's working tree stays (clean never touches
        // anything outside .p4nexus).
  ...
  describe('unhappy path', () => {
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/p4nexus/i);
  ...
  describe('CLI error handling', () => {
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // in this suite may have created MINI_REPO/.p4nexus via analyze,
      // and findRepo() walks up so any `.p4nexus` along the path still
  ...
  describe('wiki command flags', () => {
    ...
    it('wiki on non-indexed repo fails with "No P4Nexus index"', () => {
      ...
      try {
        ...
        // Must spawn outside project tree so it doesn't find parent .p4nexus
        ...
        expect(result.status).toBe(1);
        expect(result.stdout).toMatch(/No P4Nexus index found/);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
  ...
  // Requires analyze to have run first (the analyze test above populates .p4nexus/).
    ...
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('P4NEXUS_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('P4NEXUS_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
