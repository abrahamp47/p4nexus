...
All notable changes to P4Nexus will be documented in this file.
...
- **`p4nexus analyze --name <alias>`** with duplicate-name guard in the repo registry (#955)
- **`p4nexus remove <target>`** unindexes a registered repo by name or path (#664, #1003)
...
- **Configurable large-file skip threshold** — the walker's 512 KB default is now overridable via `P4NEXUS_MAX_FILE_SIZE` (KB) or `p4nexus analyze --max-file-size <kb>`. Values are clamped to the 32 MB tree-sitter ceiling, invalid inputs fall back to the default with a one-time warning, and the CLI banner reports the effective post-clamp threshold when an override is active (#991, #1044, #1045)
- **`P4NEXUS_INDEX_TEST_DIRS` opt-in** for `__tests__` / `__mocks__` traversal (#771, #1046)
...
- **CLI p4nexus markers** — `<!-- p4nexus:* -->` is only matched at section position, no longer inside code/prose (#1041, #1042)
...
- **Docker** — RC build guarded against empty `vtag`, `inputs.tag` used to detect `workflow_call` context, web builder stage now copies `p4nexus/package.json`, base image switched from alpine to debian (#983, #996, #997, #1014)
...
- p4nexus-web dependency bumps: `vite` 5.4.21 → 6.4.2 → 7.3.2 → 8.0.10 + `vitest` 4 (#1061, #1062, #1063), `lucide-react` 0.562.0 → 1.11.0 with local GitHub SVG fallback (#1038), `@langchain/anthropic` 1.3.10 → 1.3.27 (#1039), `@babel/types` (#1037)
- p4nexus-shared dependency bumps: `typescript` (#1034)
...
- **Global-install upgrade `ENOTEMPTY`** — devendored `tree-sitter-proto` install lifecycle + preinstall cleanup so `npm i -g p4nexus@latest` succeeds on top of an older install (#843, #846)
...
- p4nexus-web dependency bumps: `mermaid` 11.12.2 → 11.14.0 (#860), `tailwindcss` (#861), `jsdom` 29.0.0 → 29.0.2 (#863), `wait-on` 8.0.5 → 9.0.5 (#859), `@vitest/coverage-v8` (#864)
...
- **Runtime dependency on `file:../p4nexus-shared`** removed from the published package (#803)
...
- **`P4NEXUS_HOME` env var** to customize global directory (#746)
...
- **`.p4nexusignore` negation patterns** now respected (#654)
- **MCP setup** prefers global p4nexus binary over npx (#653)
...
### Fixed
- **`p4nexus-shared` module not found** — `p4nexus-shared` was a `file:` workspace dependency never published to npm, causing `ERR_MODULE_NOT_FOUND` when installing `p4nexus` globally. The build now bundles shared code into `dist/_shared/` and rewrites imports to relative paths (#613)
...
### Fixed
- Incomplete fix for `p4nexus-shared` bundling — `prepare` script overwrote rewritten imports during publish
...
- **Unified web & CLI ingestion pipeline** — complete architectural migration of the web app from a self-contained WASM browser app to a thin client backed by the CLI server; new `p4nexus-shared` package for cross-package type unification (#536)
...
- **`--skip-agents-md` CLI flag** — opt out of overwriting P4Nexus-managed sections in AGENTS.md and CLAUDE.md during `p4nexus analyze` (#517)
...
### Docs
- Added `p4nexus-shared` build step to web UI quick start instructions (#585)
- Added enterprise offering section to README (#579)

## [1.4.10] - 2026-03-27

### Fixed
- **MCP server install via npx** — resolve tree-sitter peer dependency conflicts that broke `npx -y p4nexus@latest mcp` (#537, #538)
...
- `p4nexus-stable-ops` added to community integrations
...
- `.gitignore` and `.p4nexusignore` support during file discovery (#231)
