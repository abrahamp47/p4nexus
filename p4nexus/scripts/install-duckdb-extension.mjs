...
async function installDuckDbExtension(extensionName) {
  ...
  const lbugMaxDbSize = parseLbugMaxDbSize(
    process.argv[3] ?? process.env.P4NEXUS_LBUG_MAX_DB_SIZE,
  );

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'p4nexus-ext-install-'));
...
installDuckDbExtension(process.argv[2] ?? process.env.P4NEXUS_LBUG_EXTENSION_NAME).catch((err) => {
